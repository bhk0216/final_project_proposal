...
renderPlotly({
  combined_df |> 
    ...
    summarise(
      total = n(),
      heart_disease = sum(num == 1),
      proportion = heart_disease / total
    ) |> 
    mutate(sex = factor(sex, levels = c(0, 1), labels = c("Female", "Male"))) |> 
    plot_ly(
      x = ~sex, y = ~proportion, type = "bar",
      color = ~sex,
      hoverinfo = "text",
      text = ~paste("Sex: ", sex, "<br>Proportion: ", round(proportion, 2))
    ) |> 
    layout(title = "Proportion of Heart Disease by Gender",
